...
export default function AdminSignupPage() {
  ...
  return (
    <div className="min-h-screen bg-[#07090e] flex items-center justify-center px-4 py-12 relative overflow-hidden">
      {/* Ambient glows */}
      <div className="absolute top-[-80px] right-[-60px] w-96 h-96 rounded-full bg-violet-500/10 blur-[120px] pointer-events-none" />
      <div className="absolute bottom-[-60px] left-[-60px] w-80 h-80 rounded-full bg-cyan-600/10 blur-[100px] pointer-events-none" />
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 text-sm mt-1">Join VinuLabs Admin Panel</p>
        </div>

        {/* Card */}
        <div className="bg-slate-900/80 border border-slate-800/80 backdrop-blur-xl rounded-2xl p-8 shadow-2xl shadow-black/40">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl
              border border-slate-700 bg-slate-800/60 hover:bg-slate-700/80
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="text-xs text-slate-500 font-mono">or sign up with email</span>
            <div className="flex-1 h-px bg-slate-800" />
          </div>

          {/* Error */}
          {error && (
            <div className="mb-4 px-4 py-3 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-400 text-xs flex items-center gap-2">
          ...
          <form onSubmit={handleSignup} className="space-y-4" noValidate>
            ...
            {/* Full Name */}
            <div>
              <label htmlFor="signup-name" className="block text-xs font-semibold text-slate-400 mb-1.5">Full Name</label>
              <div className="relative">
                ...
                <input
                  ...
                  autoComplete="name"
                  className={`w-full pl-10 pr-4 py-3 rounded-xl bg-slate-800/60 border text-white text-sm placeholder-slate-500
                    focus:outline-none focus:ring-2 focus:ring-cyan-500/60 focus:border-cyan-500/60 transition-all duration-200
                    ${name.length > 0 && name.trim().length < 2 ? 'border-rose-500/50' : 'border-slate-700'}`}
                />
              </div>
              {name.length > 0 && name.trim().length < 2 && (
                <p className="text-rose-400 text-[11px] mt-1 ml-1">Name must be at least 2 characters</p>
              )}
            </div>

            {/* Email */}
            <div>
              <label htmlFor="signup-email" className="block text-xs font-semibold text-slate-400 mb-1.5">Email address</label>
              <div className="relative">
                <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-500">
                ...
                <input
                  ...
                  className={`w-full pl-10 pr-4 py-3 rounded-xl bg-slate-800/60 border text-white text-sm placeholder-slate-500
                    focus:outline-none focus:ring-2 focus:ring-cyan-500/60 focus:border-cyan-500/60 transition-all duration-200
                    ${email.length > 0 && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) ? 'border-rose-500/50' : 'border-slate-700'}`}
                />
              </div>
              {email.length > 0 && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) && (
                <p className="text-rose-400 text-[11px] mt-1 ml-1">Enter a valid email address</p>
              )}
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-12 py-3 rounded-xl bg-slate-800/60 border border-slate-700 text-white text-sm placeholder-slate-500
                    focus:outline-none focus:ring-2 focus:ring-cyan-500/60 focus:border-cyan-500/60 transition-all duration-200"
                />
                <button
                  type="button"
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {strength > 0 && (
                      <span className={`text-[11px] font-bold font-mono ${
                        strength <= 2 ? 'text-rose-400' : strength === 3 ? 'text-yellow-400' : strength === 4 ? 'text-lime-400' : 'text-emerald-400'
                      }`}>{strengthLabel}</span>
                    )}
                  </div>
                  {/* Rules checklist */}
                    ...
                    {passwordRules.map((rule, i) => {
                      ...
                      return (
                        <div key={i} className="flex items-center gap-1.5">
                          {passed ? (
                            <svg viewBox="0 0 16 16" fill="currentColor" className="w-3 h-3 text-emerald-400 shrink-0"><path fillRule="evenodd" d="M13.78 4.22a.75.75 0 010 1.06l-7.25 7.25a.75.75 0 01-1.06 0L2.22 9.28a.75.75 0 011.06-1.06L6 10.94l6.72-6.72a.75.75 0 011.06 0z"/></svg>
                          ) : (
                            <svg viewBox="0 0 16 16" fill="currentColor" className="w-3 h-3 text-slate-600 shrink-0"><path fillRule="evenodd" d="M8 15A7 7 0 118 1a7 7 0 010 14zm0 1A8 8 0 108 0a8 8 0 000 16z"/></svg>
                          )}
                          <span className={`text-[11px] ${passed ? 'text-emerald-400' : 'text-slate-500'}`}>{rule.label}</span>
                        </div>
                      );
                    })}
                  </div>
            ...
            {/* Confirm Password */}
            <div>
              <label htmlFor="signup-confirm" className="block text-xs font-semibold text-slate-400 mb-1.5">Confirm Password</label>
              <div className="relative">
                <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-500">
                ...
                <input
                  ...
                  autoComplete="new-password"
                  className={`w-full pl-10 pr-12 py-3 rounded-xl bg-slate-800/60 border text-white text-sm placeholder-slate-500
                    ...
                    ${passwordsMatch
                      ...
                      : 'border-slate-700 focus:ring-cyan-500/60 focus:border-cyan-500/60'}`}
                />
                {/* Eye toggle */}
                <button
                  type="button"
                ...
                <span className="absolute right-3.5 top-1/2 -translate-y-1/2">
                  ...
                  {passwordsMismatch && (
                    <svg viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4 text-rose-400"><path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd"/></svg>
                  )}
                </span>
              </div>
              {/* Real-time match message */}
              {passwordsMismatch && (
                <p className="text-rose-400 text-[11px] mt-1 ml-1">Passwords do not match</p>
              )}
              {passwordsMatch && (
                <p className="text-emerald-400 text-[11px] mt-1 ml-1">✓ Passwords match</p>
              )}
            </div>
        ...
        <p className="text-center text-xs text-slate-600 mt-6">
          <Link href="/" className="hover:text-slate-400 transition-colors">← Back to VinuLabs.com</Link>
        </p>
      </div>
    </div>
  );
